...
p. Currently the library supports page view tracking (which can have a referrer from a page or a search engine) and event tracking.  See the usage section for a basic setup (It's very simple).
...
p. and then you can send tracking data with the @trackPageView@ and @trackEvent@ methods, or make your own request the @makeCustomRequest(AnalyticsRequestData)@ method.  Tracking referrals and searches is made through calling @trackPageViewFromReferrer@ and @trackPageViewFromSearch@.
